...
class GearADB(object):
    ...
    def screenshot(self, sleep=5):
        ...
        while True:
            try:
                self.push("adb shell screencap -p /data/local/tmp/tmp.png")
                ...
                time.sleep(sleep)
                os.popen("adb pull /data/local/tmp/tmp.png " + os.path.abspath(self.screenshot_path + "/screenshot_temp.png"))

                time.sleep(2)
                os.popen("adb shell rm /data/local/tmp/tmp.png")
                print('截图完成.')

                img = Image.open(r"screenshot\screenshot_temp.png").convert('L')
                return img
            except:
                sleep += 1
